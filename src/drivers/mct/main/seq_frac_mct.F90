...
contains
  ...
  subroutine seq_frac_init( infodata,         &
    ...
    !BK: gfrac is now at play, time-variant
    character(*),parameter :: fraclist_a = 'afrac:ifrac:ofrac:lfrac:lfrin:gfrac' !bk
    character(*),parameter :: fraclist_o = 'afrac:ifrac:ofrac:ifrad:ofrad:gfrac' !bk
    character(*),parameter :: fraclist_i = 'afrac:ifrac:ofrac:gfrac'             !bk
    character(*),parameter :: fraclist_l = 'afrac:lfrac:lfrin:gfrac'             !bk
    character(*),parameter :: fraclist_g = 'gfrac:lfrac'
    character(*),parameter :: fraclist_r = 'lfrac:rfrac'
...
!bk    fractions_g%rAttr(kg,:) = dom_g%data%rAttr(kf,:)  # BK: new,  use t-variant Sg_ice_covered from glc
  ...
  subroutine seq_frac_set(infodata, ice, glc, fractions_a, fractions_i, fractions_o)
    ...
    type(mct_ggrid), pointer :: dom_i
    type(mct_aVect), pointer :: g2x_g   ! BK add glc to API
    logical                  :: atm_present   ! true => atm is present
    logical                  :: ice_present   ! true => ice is present
    ...
    g2x_g => component_get_c2x_cx(glc)  ! BK add glc to API
